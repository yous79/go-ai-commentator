    ...
    def generate_commentary(self, move_idx, history, board_size=19):
        ...
        try:
            ...
            sys_inst_talk = (
                ...
                f"【状況】ツール解析により、正確な盤面データ（勝率・目数・PV）を取得しました。\n"
                f"【指示】\n"
                f"1. ユーザーの「盤面が見えない」という以前の発言は、データ取得前の手続き上のものですので、回答では言及しないでください。\n"
                f"2. 取得した解析データと知識ベースを根拠に、自信を持って解説してください。\n"
                f"3. 勝率や変化図(PV)の具体的な内容に触れながら、「なぜその手が良い/悪いのか」を論理的に説明してください。"
            )
