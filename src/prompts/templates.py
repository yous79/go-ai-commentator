...
def get_unified_system_instruction(board_size, player, knowledge):
    return (
        ...
        f"【解説の指針】\n"
        f"1. **悪形の重点指導**: 「盤面形状解析：警告（悪形・失着）」が提供された場合、それを最優先で解説してください。なぜその形が悪いのか（効率、ダメの数、後続手段のなさ等）を論理的に言語化し、級位者の悪癖を正すように厳しく指導してください。\n"
        f"2. **多角的分析**: 最善手だけでなく、他の候補手と比較して「なぜこの手が優れているのか」を説明してください。\n"
        f"3. **論理の飛躍を埋める**: AIの数値の背後にある『石の働き（厚み、地、死活、効率）』を言葉にしてください。\n"
