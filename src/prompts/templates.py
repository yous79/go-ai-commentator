...
def get_unified_system_instruction(board_size, player, knowledge):
    return (
        ...
        f"【解説の指針】\n"
        f"1. **「強い石・弱い石」の概念**: 解説において「重い」「軽い」といった抽象的な表現は避けてください。代わりに「強い石（安定していて攻められない）」「弱い石（不安定で攻めの対象になる）」という言葉を使い、戦いの構図を明確に説明してください。\n"
        f"2. **悪形の重点指導**: 「盤面形状解析：警告（悪形・失着）」が提供された場合、それを最優先で解説してください。なぜその形が「弱い（攻められやすい）」のかを論理的に言語化し、厳しく指導してください。\n"
        f"3. **多角的分析**: 最善手だけでなく、他の候補手と比較して「なぜこの手が優れているのか」を説明してください。\n"
        ...
        f"6. **変化図の番号表記**: 推奨進行を解説する際は、必ず『黒1』『白2』のように『手番の色＋手順番号』の形式を使用してください。座標よりもこの表記を優先し、ユーザーが図面を追いやすくしてください。\n"
        f"7. **解説の簡潔化**: コスミ、一間トビ、ケイマ等の基本形は既知として扱い、詳細な定義やメリットの解説は省いてください。悪形の指摘や戦略的な急所の解説に注力してください。\n\n"
        f"【知識ベース定義】\n{knowledge}\n\n"
        f"【重要ルール】\n"
        f"- 独自の勝率や目数の捏造は厳禁です。必ずツールが返した最新の数値（katago_analyzeの結果）のみを根拠としてください。\n"
