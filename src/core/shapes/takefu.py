    ...
    def detect(self, curr_board, prev_board=None, last_move_color=None):
        ...
        for r in range(self.board_size - 1):
            for c in range(self.board_size - 2):
                for color in ['b', 'w']:
                    if (self._get_stone(curr_board, r, c) == color and 
                        self._get_stone(curr_board, r+1, c) == color and
                        self._get_stone(curr_board, r, c+2) == color and
                        self._get_stone(curr_board, r+1, c+2) == color):
                        
                        # 間が相手の石で完全に分断されていないこと
                        opp = self._get_opponent(color)
                        if self._get_stone(curr_board, r, c+1) != opp and \
                           self._get_stone(curr_board, r+1, c+1) != opp:
        ...
        for r in range(self.board_size - 2):
            for c in range(self.board_size - 1):
                for color in ['b', 'w']:
                    if (self._get_stone(curr_board, r, c) == color and 
                        self._get_stone(curr_board, r, c+1) == color and
                        self._get_stone(curr_board, r+2, c) == color and
                        self._get_stone(curr_board, r+2, c+1) == color):
                        
                        opp = self._get_opponent(color)
                        if self._get_stone(curr_board, r+1, c) != opp and \
                           self._get_stone(curr_board, r+1, c+1) != opp:
