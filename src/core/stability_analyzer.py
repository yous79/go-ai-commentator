...
class StabilityAnalyzer:
    ...
    def analyze_to_facts(self, board, ownership_map) -> list[InferenceFact]:
        ...
        for r in results:
            ...
            if r['status'] == 'dead':
                severity = 4
                status_msg = "は既に死んでいるか、戦略的価値の低い『カス石』です。助けようとせず、捨て石として活用するか、放置して大場に回るべきです。"
            elif r['status'] == 'critical':
                severity = 5
                status_msg = "が極めて危険（生存確率 20%未満）です。助ける価値があるなら即座の救援が必要ですが、重い場合は捨てる判断も必要です。"
            elif r['status'] == 'weak':
                severity = 4
                status_msg = "が弱く、攻撃の対象になっています。この石の補強は急場です。"
    ...
    def analyze(self, board, ownership_map):
        ...
        for color_long, stones in groups:
            # color_long は 'black', 'white' などの文字列
            ...
            for p in stones:
                ...
                if idx < len(ownership_map):
                    ...
                    stability = val if color_code == 'b' else -val
