...
- [ ] **対局操作のコマンドパターン化（強化）**: Undo/Redo の完全対応と履歴の可視化。
- [ ] **設定管理のクラス化 (ConfigManager)**: グローバル変数を廃止し、設定変更イベントをバスに流す仕組みへ移行。
- [ ] **盤面描画のレイヤー化**: `GoBoardRenderer` を分割し、描画ロジックを拡張可能にする。
...
---
*最終更新: 2026-01-17 (Rev 41.0 Updated)*
