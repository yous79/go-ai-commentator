...
- [ ] **形状検知の単体テスト化 (Unit Testing)**: アスキーアート等を用いた盤面データを `detect()` 関数に渡すテストスイートを作成し、コマンド一発で論理検証可能にする。（Rev 19.0でプロトタイプ導入済）
...
- [ ] **宣言的パターン定義 (Declarative Patterns)**: 座標計算を直接書くのではなく、相対座標と状態（自分・敵・空）の定義リストから自動検知する仕組みへ移行。バグの温床となるループ処理を隠蔽。
...
- [ ] **座標Value Object (Pointクラス)**: 座標を単なるタプルではなく、`p1 + p2` のように演算可能なクラスとして定義し、計算ミスを型レベルで防止。
- [ ] **APIサーキット・ブレーカー**: 通信エラーが連続した際にリクエストを一時遮断し、スレッド累積による「システムが重くなる」現象をインフラ層で防止。
