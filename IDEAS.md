...
- [ ] **盤面座標・石色の正規化レイヤー**: `UnifiedBoard` クラスと `Color` Enum の導入により、`row/col` や `b/w` の表記揺れに起因するバグを根絶。
- [ ] **UIコンポーネントの細分化**: `InfoView` を `AnalysisTab` や `DictionaryTab` 等へ分割し、コードの肥大化を抑制。
- [ ] **イベント駆動型アーキテクチャへの移行**: 巨大な `callbacks` 辞書の代わりに Pub/Sub モデルを導入し、コンポーネント間の結合度を低減。
- [ ] **ロギングシステムの標準化**: 散在する `print` を `logger` へ置き換え、ログレベルによる適切なデバッグ管理を実現。
