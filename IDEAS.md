...
- [x] **用語からの具体例画像生成**: 用語IDからSGFテンプレート（またはAI生成）を用いて、正確な盤面画像を動的に生成。 (Rev 25.0 完了)
- [x] **UIへの用語辞典統合**: `test_play` およびメインUIに用語辞典タブを追加し、解説の閲覧と具体例画像の表示を可能にした。 (Rev 26.0 完了)
...
- [x] **知能の構造化 (InferenceFact)**: 解析事実をオブジェクト化し、重要度順にソート（トリアージ）する仕組み。 (Rev 34.0 完了)
- [x] **シミュレーション・コンテキストの導入**: 未来予測の盤面状態を一元管理する SimulationContext の導入。 (Rev 34.0 完了)
...
- [x] **APIサーキット・ブレーカー**: 通信エラーが連続した際にリクエストを一時遮断。 (Rev 24.0 完了)
- [x] **堅牢な座標変換**: 空文字やパスに対する防御を備えた、IndexErrorの起きない座標処理。 (Rev 33.0 完了)
- [x] **盤面サイズ動的同期**: 9x9, 13x13, 19x19の混在局面でもIndexErrorを起こさない、動的なサイズ検知と同期。 (Rev 34.0 完了)

---
*最終更新: 2026-01-16 (Rev 34.0 Updated)*
