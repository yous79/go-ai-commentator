...
- [x] **局面比較ツール (Batch Comparison)**: A 図と B 図の解析結果を一度に取得し差分計算。 (Rev 46.0 完了)
- [x] **構造化データの返却**: `detect_shapes` が JSON 形式で詳細なメタデータを返すように強化。 (Rev 47.0 完了)
- [x] **解消座標 (Remedy) の提供**: 悪形検知時に「手入れすべき急所」を AI に提示。 (Rev 47.0 完了)
- [ ] **時間軸タグ (Temporal Tagging)**: 形状が「現局面での事実」か「PV内での予測」かを区別。
- [ ] **コンテキスト・オフローディング**: MCP サーバー側で盤面セッションを維持し、通信量を削減。
- [ ] **動的な指導人格の切り替え (Dynamic Profile)**: 序盤・中盤・終盤で指導指針を動的に変更。

---
*最終更新: 2026-01-19 (Rev 47.0 Updated)*