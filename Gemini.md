# 囲碁AI解説システム 統合仕様書 (Gemini.md) - Rev. 31.0
...
- **学術的根拠の重視**: 囲碁AI解説に関する先行研究や論文の知見を取り入れ、科学的に裏付けられたシステムを構築する。
...
知識ベースは、メタデータ、論文の知見、および「正確なお手本」を提供する能力を備えた。
- **メタデータ (metadata.json)**: 各用語に `importance`, `related_terms`, `tags` を付与。
- **論文ナレッジ (knowledge/00_papers/)**: 囲碁AI解説に関する先行研究や論文のエッセンスを蓄積し、システム設計や解説の質の向上に活用する。
...
## 6. AI解説生成：事実先行・人格統合プロトコル (Fact-First & Persona Integration)
- **確定事実の優先**: 解析データ、形状検知結果、安定度分析をプロンプト最上部に配置。
- **人格の参照**: Gemini は解説生成にあたり、プロジェクト内の **`Gemini_Persona.md`** を常に参照する。
- **医学用語の排除**: 直接的な医学用語の乱用を厳禁とし、囲碁の言葉による論理的・効率的なスタイルを維持する。
...
- **エージェントの振る舞い**: エージェント（私）は、常に `Gemini_Persona.md` の理念（論理、効率、再現性）を適宜参照し、囲碁ドクターの人格に沿った誠実かつ知的な振る舞いを行うものとする。
