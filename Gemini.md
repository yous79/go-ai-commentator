# 囲碁AI解説システム 統合仕様書 (Gemini.md) - Rev. 26.0
...
- **構造적条件**: 並走する攻撃側（自分）の石の助けを得て、防御側（相手）の「厳密に2つ」並んだ石の頭を叩くこと。
- **三目以上の除外**: 連結が3つ以上（三目の頭以上）の場合は、石の強度が異なるため検知対象から除外する。
- **先制の原則**: 叩いた石（最新手）の反対側が「空点」であることを必須条件とし、混戦と区別する。

### 2.3 サカレ形 (Sakare Gata) の完全分断証明 [REVISED]
サカレ形の検知条件を「連絡の権利」の喪失という観点から厳密化した。
- **物理的遮断の完了**: 自分の2石（一間トビ、またはケイマ）の間の最短連絡経路となる**全ての共通近傍交点**が、相手の石で埋め尽くされていること。
- **一間トビの条件**: 相手による「3石の壁」による封鎖。
- **ケイマの条件**: 相手による「2石の壁」による封鎖。
- **ワリコミの除外**: 相手の石が1石のみの状態は、まだ連絡の余地があるため「ワリコミ」として区別し、サカレ形からは除外する。
...
## 4. 構造化知識ベースと可視化 (Structured Knowledge & Visualization) [NEW]
...
- **用語ビジュアライザー (TermVisualizer)**: 
    ...
    - **テンプレート優先**: `example.sgf` に基づく正確なレンダリング。
    - **AIフォールバック**: テンプレート不在時のみ、**Gemini 3 Flash Preview** によるSGF生成を試行。
...
- **確定事実の優先**: 解析データ、形状検知結果、安定度分析、および関連知識メタデータをプロンプト最上部に配置。
- **Gemini 3 Flash Preview** は提供された正確なデータに基づき、戦略的かつ論理的な言語化を行う。
