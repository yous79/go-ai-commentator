# 囲碁AI解説システム 統合仕様書 (Gemini.md) - Rev. 43.0

## 0. モデル名称・認識に関する絶対規約 (Model Identification Protocol) - [改訂不可]
1.  **最新モデルの正確な認識**: 本システムが使用する最新の戦略的知性モデルは **Gemini 3 Flash Preview** (および内部的に最適化された **gemini-2.0-flash-001**) である。
2.  **実行プロセスにおける絶対遵守事項 (0.4)**:
...
- **学術的エビデンス**: 論文レベルの知見（手順情報を用いた用語付与、勝率推移の言語化等）をロジックの根底に置く。
...
- **推論の構造化**: AIは「生データ」ではなく、トリアージされた「構造化された事実 (InferenceFact)」を元に解説文を構築する。
