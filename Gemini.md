# 囲碁AI解説システム 統合仕様書 (Gemini.md) - Rev. 49.0
...
- **学術적根拠の重要性**: 囲碁AI解説に関する先行研究や論文の知見（手順情報、勝率推移、着手価値等）を取り入れ、科学的に裏付けられたシステムを構築する。
...
- **時間軸タグ (Temporal Tagging)**: 全ての解析事実に `TemporalScope` (Immediate / Existing / Predicted) を付与。AIは「今起きた変化」「以前からの懸案」「将来の予測」を明確に区別し、時系列に基づいた正確な解説を生成する。
