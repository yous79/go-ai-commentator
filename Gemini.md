# 囲碁AI解説システム 統合仕様書 (Gemini.md) - Rev. 50.0
...
Ownershipデータから地の確定率を算出し、85%以上（閾値0.9）で「終盤」と判定。この情報を各解析ツールのレスポンスに含めることで、AIの戦略的トーンを自動調整させる。
...
## 5. インフラストラクチャ
- **事実プロバイダ・パターン (Fact Provider Pattern)**: 局面解析の各責務（形状、安定度、緊急度等）を独立した `BaseFactProvider` 継承クラスとして分離。`AnalysisOrchestrator` はこれらのプロバイダを統括する役割に特化し、高い拡張性と保守性を実現する。
- **モジュール化MCPサーバ**: MCPサーバを `System`, `Knowledge`, `Analysis` の3つの特化クラスに分割し、責務を明確化。
- **コンテキスト・オフローディング (Context Offloading)**: MCPサーバ側で対局状態を保持する `SessionManager` を導入。クライアントは `sync_session` ツールで文脈をオフロードし、以降の解析リクエストでは履歴の送信を省略する。
- **GeminiModel 抽象化層**: OpenRouter 経由の呼び出しや並列実行 (`call_parallel`)、リトライ、指数バックオフを標準実装。
- **Atomic Storage**: 全てのJSON保存は「一時ファイル保存 → リネーム」の操作で行い、ファイル破損を防止する。

## 6. AI解説プロトコル：事実先行 (Fact-First)
- **推論の構造化**: AIは提供された「構造化された事実 (InferenceFact)」を元に解説文を構築する。
- **最新手の優先評価 (Last Move Evaluation)**: 解説の冒頭で、直前の着手（最新手）の是非（効率、働き、悪形の有無）を論理的に評価することを義務付ける。
- **時間軸タグ (Temporal Tagging)**: 全ての解析事実に `TemporalScope` (Immediate / Existing / Predicted) を付与。AIは「今起きた変化」「以前からの懸案」「将来の予測」を明確に区別する。

## 7. UI/UX アーキテクチャ
- **イベント駆動 (Event-Driven)**: `EventBus` による完全な疎結合。Viewはイベントを購読して自律的に更新される。
- **ナビゲーションの高度化**: 左右キーによる Undo/Redo のサポート、Redoバッファの実装、`bind_all` によるグローバル操作。

## 8. 検証・品質保証
- **二重検証義務**: 
    1. `python src/utils/check_startup.py` による起動確認。
...
- **数値監査**: 生成された解説文内の数値を事実データと照合し、不一致がある場合は警告を行う。